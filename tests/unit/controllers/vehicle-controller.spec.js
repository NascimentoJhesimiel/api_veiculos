...
describe("Vehicle Controller Tests", () => {
  ...
  describe("getVehicleById", () => {
    ...
    test("Should return status 404 when no vehicles data found", async () => {
      ...
      expect(mockRead).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({ error: 'No vehicles data found' });
    });
    ...
    test("Should return status 500 on database error", async () => {
      ...
      expect(mockRead).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ error: 'Error to fetch vehicle' });
    });
  });
  ...
  describe("createVehicle", () => {
    ...
    test("Should return status 500 on creation error", async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Error creating vehicle' });
  ...
  describe("updateVehicle", () => {
    ...
    test("Should return status 500 on update error", async () => {
      ...
      expect(mockUpdate).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Error updating vehicle' });
    });
  });
  ...
  describe("deleteVehicle", () => {
    ...
    test("Should return status 500 on deletion error", async () => {
      ...
      expect(mockDelete).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Error deleting vehicle' });
    });
  });
});